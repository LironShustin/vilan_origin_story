...
void is_realistic(BOOL* realistic, char* story_file_name) {
    ...
    answer[0] = '0';
    while (answer[0] == '0') {
        print_colored("\nIs your story realistic or fictional?\n", YELLOW);
...
void are_you_happy_with_your_result(int r, int match_count, const char* database, int* match_lines, int n, Output_Vilain_Struct* output, int* run, int* print) {
    char answer = '0';
    while (answer == '0') {
        print_colored("\nAre you happy with the villain/antagonist generated?\n", YELLOW);
        print_colored("[1] Yes\n[2] No\n", CYAN);
        scanf_s(" %c", &answer, 1);
        int ch;
        while ((ch = getchar()) != '\n' && ch != EOF) {}
...
void generate_another(int r, int match_count, const char* database, int* match_lines, int n, Output_Vilain_Struct* output, int* run, int* print) {
    ...
    char answer = '0';
    while (answer == '0') {
        print_colored("\nDo you want to generate another villain?\n", YELLOW);
        print_colored("[1] Yes\n[2] No\n", PURPLE);
        scanf_s(" %c", &answer, 1);
        int ch;
        while ((ch = getchar()) != '\n' && ch != EOF) {}
        if (answer == 'y' || answer == 'Y' || answer == '1') {
            answer = '0';
            while (answer == '0') {
                print_colored("\nFantastic!, do you want to keep the same parameters?\n", YELLOW);
                print_colored("[1] Yes\n[2] No\n", BLUE);
                scanf_s(" %c", &answer, 1);
				clear_console();
                int ch;
